...
export function BlushHavenDetail({product, isDraftPreview}: Props) {
  ...
  const ui = useTranslations("ProductDetail.blushHaven");
  ...
  return <div className="overflow-clip bg-[#f8f7f3] text-[#202725]">
    <section className="border-b border-black/10">
      <div className={`${sectionClass} grid gap-7 pb-14 pt-7 lg:min-h-[790px] lg:grid-cols-[0.75fr_1.25fr] lg:items-center lg:gap-16 lg:py-16`}>
        <div className="order-2 lg:order-1">
          <div className="flex flex-wrap items-center gap-3 text-xs font-medium tracking-[0.16em] text-[#8f6b6b]"><span>{ui("eyebrow")}</span>{isDraftPreview ? <span className="border border-[#d7aaaa] bg-[rgba(250,248,243,0.72)] px-2.5 py-1 text-[0.64rem] text-[#8f5d65] backdrop-blur-sm">{t("draftPreview")}</span> : null}</div>
    ...
    {scene ? <section className="bg-[#f1ece8] py-12 sm:py-16"><div className={sectionClass}><div className="grid gap-8 lg:grid-cols-[1.12fr_0.88fr] lg:items-center lg:gap-16">
      ...
      <div><p className="text-xs font-medium tracking-[0.18em] text-[#9a7477]">{ui("livingEyebrow")}</p><h2 className="mt-4 text-3xl font-medium tracking-[-0.05em] sm:text-5xl">{ui("livingTitle")}</h2><div className="mt-7 flex flex-wrap gap-2">{scenes.map((item, index) => <button aria-pressed={sceneIndex === index} className={`product-detail-control min-h-11 border px-4 text-sm ${sceneIndex === index ? "border-[#b9878d] bg-[rgba(250,248,243,0.76)] text-[#81575d]" : "border-black/10 text-[#5e6864]"}`} key={item.src} onClick={() => setSceneIndex(index)} type="button">{ui(`scenes.${index}`)}</button>)}</div><p className="mt-5 max-w-[34rem] text-sm leading-7 text-[#66706e]">{scene.caption}</p></div>
    ...
    <section className="bg-[#f1ece8] py-16 sm:py-24"><div className={sectionClass}><div className="grid gap-9 lg:grid-cols-[0.82fr_1.18fr] lg:gap-20">
      <div><p className="text-xs font-medium tracking-[0.18em] text-[#9a7477]">{ui("designEyebrow")}</p><h2 className="mt-4 text-3xl font-medium tracking-[-0.05em] sm:text-5xl">{ui("structureTitle")}</h2><p className="mt-6 text-base leading-8 text-[#5e6864]">{product.modules[0]?.description}</p><button aria-expanded={showDesignNotes} className="product-detail-control mt-7 inline-flex min-h-11 items-center gap-3 border-b border-[#af8589] text-sm font-medium text-[#81575d]" onClick={() => setShowDesignNotes((value) => !value)} type="button">{showDesignNotes ? "−" : "+"} {showDesignNotes ? t("collapseModules") : t("allModules")}</button>{showDesignNotes ? <p className="mt-5 text-sm leading-7 text-[#66706e]">{product.moduleNote}</p> : null}</div>
      <div className="border-t border-[#cfbfc0] pt-5 lg:pt-12"><p className="text-xs tracking-[0.16em] text-[#7a7672]">{ui("sizeTitle")}</p><p className="mt-4 whitespace-pre-line text-3xl font-medium tracking-[-0.04em] sm:text-4xl">{product.dimensionsDisplay}</p><p className="mt-5 max-w-[38rem] whitespace-pre-line text-sm leading-7 text-[#66706e]">{product.dimensionsNote}</p><p className="mt-6 text-xs leading-6 text-[#7a7672]">{product.moduleNote}</p></div>
    </div></div></section>
    ...
    {materialImage ? <section className={`${sectionClass} py-16 sm:py-24`}><div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-20">
      <div><p className="text-xs font-medium tracking-[0.18em] text-[#9a7477]">{ui("materialEyebrow")}</p><h2 className="mt-4 text-3xl font-medium tracking-[-0.05em] sm:text-5xl">{t("designDetails")}</h2><p className="mt-6 max-w-[38rem] text-base leading-8 text-[#5e6864]">{product.materialDescription}</p><div className="mt-7 flex flex-wrap gap-2">{materialImages.map((item, index) => <button aria-pressed={materialIndex === index} className={`product-detail-control min-h-11 border px-4 text-sm ${materialIndex === index ? "border-[#b9878d] bg-[rgba(250,248,243,0.76)] text-[#81575d]" : "border-black/10 text-[#5e6864]"}`} key={item.src} onClick={() => setMaterialIndex(index)} type="button">{ui(`materials.${index}`)}</button>)}</div><p className="mt-5 max-w-[38rem] text-xs leading-6 text-[#747c79]">{product.materialNote}</p></div>
